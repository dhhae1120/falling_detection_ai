...
class Shift_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        nn.init.kaiming_normal_(self.downsample.weight, mode='fan_out')
        bn_init(self.bn2, 1)
...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3):
        ...
        self.Linear_weight = nn.Parameter(torch.zeros((in_channels*vertex)//self.group ,(out_channels*vertex)//self.group,requires_grad=True,device='cuda'),requires_grad=True)
        nn.init.normal_(self.Linear_weight, 0,math.sqrt(1.0 / ((out_channels*vertex)//self.group)))
        ...
        self.Feature_Mask = nn.Parameter(torch.ones(1,vertex,in_channels, requires_grad=True,device='cuda'),requires_grad=True)
        nn.init.constant_(self.Feature_Mask, 0)


        self.bn = nn.BatchNorm1d(vertex*out_channels)
        self.relu = nn.ReLU()
        ...
        index_array = np.empty(vertex*in_channels, dtype=int)
        for i in range(vertex):
            for j in range(in_channels):
                index_array[i*in_channels + j] = (i*in_channels + j + j*in_channels)%(in_channels*vertex)
        self.shift_in  = nn.Parameter(torch.as_tensor(index_array, dtype=torch.long), requires_grad=False)

        index_array = np.empty(vertex*out_channels, dtype=int)
        for i in range(vertex):
            for j in range(out_channels):
                index_array[i*out_channels + j] = (i*out_channels + j - j*out_channels)%(out_channels*vertex)
        self.shift_out = nn.Parameter(torch.as_tensor(index_array, dtype=torch.long), requires_grad=False)
...
class TeacherModel(nn.Module):
    def __init__(self, num_class=n_class, num_point=vertex, num_person=person, in_channels=3):
...
class Model(nn.Module):
    def __init__(self, teacher_model=None, num_class=n_class, num_point=vertex, num_person=person, graph=None, graph_args=dict(), in_channels=3, distill_weight=0.0):
        self.distill_weight = float(distill_weight)


        super(Model, self).__init__()
    
        self.t_net = TeacherModel(num_class=num_class, num_point=num_point,
                                  num_person=num_person, in_channels=in_channels)
        self.s_net = StudentModel(num_class=num_class, num_point=num_point,
                                  num_person=num_person, in_channels=in_channels)
        ...
        for i, margin in enumerate(margins):
            self.register_buffer(f"margin{i+1}", margin.unsqueeze(1).unsqueeze(2).unsqueeze(0).detach())


        self.use_teacher = False
        if teacher_model and isinstance(teacher_model, str) and os.path.isfile(teacher_model):
            weights = torch.load(teacher_model, map_location="cpu")
            weights = OrderedDict([[k.split("module.")[-1], v] for k, v in weights.items()])
            try:
                self.t_net.load_state_dict(weights, strict=False)
            except Exception:
                state = self.t_net.state_dict()
                state.update({k: v for k, v in weights.items() if k in state})
                self.t_net.load_state_dict(state, strict=False)
    ...
    def forward(self, x, epoch):
        if self.use_teacher:
            ...
            # Connectors 개수와 feature 개수 맞춰서 안전 처리
            feat_num = min(len(t_feats), len(s_feats), len(self.Connectors))
    
            loss_distill = 0
            for i in range(feat_num):
                s_feats[i] = self.Connectors[i](s_feats[i])
                margin = getattr(self, f'margin{i+1}')
                loss_distill += distillation_loss(s_feats[i], t_feats[i].detach(), margin) / (2 ** (feat_num - i - 1))
    
            return s_out, loss_distill
        else:
            # teacher 없음: distill loss = 0 (올바른 device로)
            _, s_out = self.s_net.extract_feature(x, epoch)
            zero = torch.zeros((), device=s_out.device)
            return s_out, zero